...
class GenericStatementParser : BillStatementParser {
    ...
    private val dueKeyword = Regex("""(?i)\b(?:total\s+(?:amt\s+)?due|amount\s+due|outstanding\s+(?:amount|balance)|total\s+outstanding)\b""")

    private val totalDuePattern = Regex("""(?i)(?:total\s+(?:amt\s+)?due|amount\s+due|outstanding\s+(?:amount|balance))\s*[:\-]?\s*(?:rs\.?|inr|sar|aed|usd|gbp|eur)?\s*([\d,]+\.?\d*)""")
    private val minDuePattern = Regex("""(?i)min(?:imum)?\s+(?:amt\s+|amount\s+)?(?:due|payment)\s*[:\-]?\s*(?:rs\.?|inr|sar|aed|usd|gbp|eur)?\s*([\d,]+\.?\d*)""")
