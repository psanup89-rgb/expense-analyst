...
**Phase 2**: Partially started (F11 mostly complete, F12 complete, F13 complete, Loans/Lent complete, Auto-save + Needs Review complete, Reimbursement Tracking complete, BNPL/Split Payments complete, Transfer Classification complete, Spent/Received Breakdown complete, Loan Legs complete)
...
| Loans/Lent Tracking | ✅ Complete | New `:feature:loans` module. Track money lent to others (PENDING/SETTLED), WorkManager reminders with custom datetime, linked transactions (see Loan Legs below) — settlement no longer fabricates an INCOME+Refund row. Entry: Settings → "Loans & Lending". DB v18: `lent_items` table. (Note: referred to as "F15" in `STATUS.md`/`HANDOFF.md`, overlapping the Cloud Backup number above — this table's F-numbers have never been fully reconciled between docs.) |
...
| Transfer Classification | ✅ Complete | Each TRANSFER is marked *sent to someone else* (counts as Spent), *moved to my own account* (never counts; own "Own Transfers" bucket in Analytics) or *received from someone else* (counts as Received). Unclassified transfers count toward nothing and appear in Needs Review with inline buttons. Classification is remembered per recipient name (exact match, forward-only). `domain/util/SpendClassifier` is now the single source of truth for every total (list, Analytics, Budget). DB v27. |
| Spent / Received Breakdown | ✅ Complete | Tapping Spent or Received on the home card opens a sheet showing how the figure was derived, plus a "Not counted" section (own-account, unclassified, loans, refunds) with a route to Review. Figures come from the same breakdown the card uses, so they cannot disagree. |
| Loan Legs | ✅ Complete | Any transaction can be linked to a loan from its detail screen as money lent out or a repayment; linked rows count toward neither Spent nor Received. Supports split lending/repayment and mixed currencies. The loan detail screen lists its linked transactions. DB v28 (`expenses.loan_id`). |
| Tag Management | ✅ Complete | Settings → Manage Tags: usage counts, search, add, rename (collision → offer merge), merge, delete (move to another tag, or remove completely), and a per-tag expense list with a Spent total. Tag names are case-insensitive. No DB migration. |
